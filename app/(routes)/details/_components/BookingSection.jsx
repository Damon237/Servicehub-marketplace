  ...
  return (
    <div>
      <Sheet onOpenChange={(open) => { 
        ...
        {/* Adjusted w-screen for mobile and w-full for sm screens to ensure no overflow */}
        <SheetContent className="overflow-y-auto w-screen sm:w-full sm:max-w-md bg-white p-4 sm:p-6">
          <SheetHeader className="pb-4 sm:pb-6 border-b">
            <SheetTitle className="text-xl sm:text-2xl font-black text-slate-800 tracking-tighter">Book a Professional</SheetTitle>
            <SheetDescription className="text-slate-500 italic text-sm">
          ...
          <div className='py-6 sm:py-8 space-y-6 sm:space-y-8'>
            <div className='flex flex-col gap-3 items-baseline relative'>
                <div className='flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-[10px] sm:text-xs font-bold uppercase'>
                ...
                <div className='relative w-full overflow-hidden'>
                  <Calendar
                    ...
                    className="rounded-xl border p-2 sm:p-4 bg-white w-full flex justify-center"
                  />

                  {showPaymentModal && (
                    <div className="absolute inset-0 z-20 bg-white/95 flex flex-col justify-center p-4 sm:p-6 animate-in fade-in duration-200 rounded-xl">
                      <h3 className="text-lg font-black text-slate-800 mb-4 text-center">Select Operator</h3>
                      <div className="grid gap-3">
                        <button onClick={() => handleSimulatedPayment('MTN')} className="flex items-center justify-between p-3 sm:p-4 border-2 border-yellow-400 rounded-xl hover:bg-yellow-50">
                        ...
                        <button onClick={() => handleSimulatedPayment('Orange')} className="flex items-center justify-between p-3 sm:p-4 border-2 border-orange-500 rounded-xl hover:bg-orange-50">
            ...
            <div className='bg-slate-50 p-4 sm:p-6 rounded-2xl border border-dashed border-slate-200 space-y-4'>
                ...
                <div className='pt-4 border-t border-slate-200 space-y-3'>
                    <div className='flex justify-between'>
                        <span className='text-xs sm:text-sm text-slate-500'>Base Rate (3 days)</span>
                        <span className='text-xs sm:text-sm font-bold text-slate-800'>2,000 XAF</span>
                    </div>
                </div>
          ...
          <SheetFooter className="mt-4 pb-10">
            <div className='w-full space-y-3'>
                <div className='flex gap-2'>
                    <SheetClose asChild>
                        <Button variant="outline" className="flex-1 rounded-xl py-6 sm:py-2" disabled={isLoading || isPaying}>
                            Cancel
                        </Button>
                    </SheetClose>
                    <Button 
      ...
      {showRatingModal && (
        <div className="fixed inset-0 bg-black/60 z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 sm:p-8 w-full max-w-[280px] sm:max-w-xs shadow-2xl text-center animate-in zoom-in">
            ...
            <Button 
              className="w-full mt-4 rounded-xl border-2 border-slate-200 py-6 sm:py-2" 
              variant="outline" 
              onClick={() => {
