...
function BookingHistoryList({ bookingHistory, type }) {
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 gap-4 px-2 sm:px-0 mt-5'>
      {bookingHistory?.length > 0 ? bookingHistory.map((booking, index) => {
        
        // FINAL FIX: Determine if businessList is an Array or a single Object
                ...
                <div className='flex flex-col gap-2 w-full'>
                  ...
                  <div className='space-y-1'>
                    ...
                    <h2 className='flex gap-2 text-gray-500 items-center text-sm'>
                      ...
                      Date: <span className='text-black font-semibold'>{booking.date}</span>
                    </h2>
                    <h2 className='flex gap-2 text-gray-500 items-center text-sm'>
                      <Clock className='text-primary h-4 w-4' /> 
                      Time: <span className='text-black font-semibold'>{booking.time}</span>
                    </h2>
                  </div>
                </div>
              </div>
        ...
        <div className='col-span-full text-center p-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 mt-5'>
          {/* Use the exact same text you had before to satisfy Next.js hydration */}
